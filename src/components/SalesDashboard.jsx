      ...
      <style>{`
        ...
        * { font-family: 'Tajawal', 'Cairo', sans-serif; box-sizing: border-box; }
        ...
        @media print {
          ...
          .printable-report { box-shadow: none !important; border: 1px solid #cbd5e1 !important; }
          .printable-report, .printable-report * { color: #0F172A !important; background-color: #ffffff !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          .printable-report th, .printable-report td { border-color: #cbd5e1 !important; }
          .printable-report .keep-accent { background-color: inherit !important; }
        }
      `}</style>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 text-[11px] font-semibold" style={{ color: sub }}>
                <span className="w-1.5 h-1.5 rounded-full pulse-dot" style={{ background: sheetUrl ? PALETTE.success : PALETTE.danger }} />
                {sheetUrl
                  ? (lastUpdate ? <>مباشر · {fmtDate(lastUpdate)} · آخر تحديث {fmtTime(lastUpdate)}</> : (loading ? "جاري التحميل..." : "لا توجد بيانات في الشيت"))
                  : "غير متصل بجوجل شيت"}
              </div>
            </div>
          </div>
        ...
        {tab === "dashboard" && (
          <>
            ...
            {missingEmployees.length > 0 && (
              <div
                className="fade-up flex items-center gap-3 rounded-2xl px-4 py-3.5 mb-5"
                style={{
...
<ResponsiveContainer width="100%" height={280}>
                  <BarChart data={byEmployee} layout="vertical" margin={{ top: 4, left: 8, right: 56, bottom: 4 }} barCategoryGap="24%">
                    <CartesianGrid strokeDasharray="3 6" stroke={dark ? "#1B2436" : "#E9ECF3"} horizontal={false} />
                    <XAxis type="number" hide />
                    <YAxis type="category" dataKey="name" tick={{ fill: text, fontSize: 13, fontWeight: 700 }} axisLine={false} tickLine={false} width={112} interval={0} />
                    <Tooltip cursor={{ fill: dark ? "rgba(255,255,255,0.03)" : "rgba(15,23,42,0.03)" }} content={<ChartTooltip dark={dark} />} />
                    <Bar dataKey="total" name="المبيعات" radius={[0, 12, 12, 0]} maxBarSize={28}>
                      ...
                      <LabelList dataKey="total" position="right" formatter={(v) => `${fmt(v)} SAR`} style={{ fill: text, fontSize: 13, fontWeight: 800 }} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
</div>
        ...
        {tab === "reconciliation" && (
          <div className="fade-up printable-report rounded-2xl p-6 md:p-8" style={{ background: "#fff", border: "1px solid rgba(15,23,42,0.08)", boxShadow: "0 2px 16px rgba(15,23,42,0.05)" }}>
            ...
            <div className="no-print flex flex-wrap items-center justify-between gap-3 mb-6">
              <div className="flex items-center gap-2">
                <label className="text-xs font-bold" style={{ color: "#64748B" }}>اسم المنشأة على التقرير:</label>
                <input
                  defaultValue={companyName}
                  onBlur={(e) => saveCompanyName(e.target.value)}
                  placeholder="مثال: مؤسسة الإدارة المركزية"
                  className="text-sm rounded-xl py-2 px-3 outline-none font-semibold"
